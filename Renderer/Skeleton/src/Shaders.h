...
namespace skel
{
	...
	struct ShaderDescriptorInformation
	{
		...
		void CreateDescriptorSets(VkDevice& _device, skel::BaseShader& _shaderInfo)
		{
	...
	{
	private:
		const char* fileSubdirectory;	// The file(s) directory in the shader resource file

		std::vector<BufferComponent*> staticBufferInfo;
		std::vector<TextureComponent*> staticTextureInfo;

	public:
		UShader(const std::vector<VkDescriptorSetLayoutBinding>& _bindings) {}
		~UShader() {}
